...
class SimpleEmbeddingService:
    """Simplified embedding service using basic vector operations"""
    
    def __init__(self):
        # Use a simple embedding dimension
        self.embedding_dim = 384  # Standard for sentence-transformers
        self.embedding_cache = {}
        
        logger.info(f"Simple embedding service initialized with dimension: {self.embedding_dim}")
    ...
    def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            for i, word in enumerate(words[:50]):  # Limit to first 50 words
                if word:
                    # Use word hash to create pseudo-embedding
                    word_hash = hash(word) % 1000
                    # Map hash to embedding dimension
                    for j in range(min(self.embedding_dim // 50, 7)):
                        idx = (i * 7 + j) % self.embedding_dim
                        embedding[idx] = (word_hash % 100 - 50) / 50.0
...
# Global simple embedding service instance
