...
import { Form, Input, Button, App, Space, DatePicker, Checkbox, Tabs, Popover, Badge, Select } from "antd";
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const formattedData = initialData.map((exp: any) => {
        ...
        const rawBullets = exp.description || exp.bulletPoints || [];
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      const formattedData = (values.experiences || []).map((exp: any) => {
        ...
        const bulletPoints = (exp.bulletPoints || []).map((bp: any) => ({
          id: bp.id,
          text: bp.text || '',
          isActive: bp.isActive !== undefined ? bp.isActive : true,
          type: bp.type || 'bullet',
                ...
                return {
                  ...
                  children: (
                    <div className="pt-4 px-1">
                      ...
                      <div className="mb-4">
                        <div className="mb-2 font-medium text-sm text-gray-400">Highlights & Achievements</div>
                        <Form.List name={[name, "bulletPoints"]}>
                          {(bpFields, { add: addBp, remove: removeBp }) => (
                            <div>
                              {bpFields.map(({ key: bpKey, name: bpName, ...bpField }) => {
                                const bulletData = form.getFieldValue(["experiences", name, "bulletPoints", bpName]) || {};
